...
`npm test` valida com uma fixture reprovada que o portal nunca apresente status aprovado quando houver falhas. Depois da geracao, `npm run validar-portal` compara os totais exibidos com o `summary.json` e interrompe a pipeline se houver divergencia. Em pull requests, o portal registra numero da PR, branch de origem, Head SHA e link da execucao.
